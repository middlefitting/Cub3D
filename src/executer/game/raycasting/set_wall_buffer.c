...
void	set_wall_buffer_logic(t_window *window, t_ray_info ray_info, t_wall_info *wall_info, int i)
{
	double	step;
	double	tex_pos;
	int		j;
	...
	// step = 1.0 * wall_info->texture->height / ray_info.line_height;
	tex_pos = (wall_info->draw_start - GAME_HEIGHT / 2 + ray_info.line_height / 2) * step;
	...
	while (j <= wall_info->draw_end)
	{
		...
		color = wall_info->texture->texture[wall_info->texture->width * tex_y + wall_info->tex_x];
		// 뺄까말까 고민하기
		// if (ray_info.side == 1)
