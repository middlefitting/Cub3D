...
void	set_side_dist(t_ray ray, t_ray_info *ray_info)
{
	...
	{
		ray_info->step_x = -1;
		ray_info->side_dist_x = (ray.pos_x - ray_info->map_x) * ray_info->delta_dist_x;
	}
	else
	{
		ray_info->step_x = 1;
		ray_info->side_dist_x = (ray_info->map_x + 1.0 - ray.pos_x) * ray_info->delta_dist_x;
	}
	if (ray_info->ray_dir_y < 0)
	{
		ray_info->step_y = -1;
		ray_info->side_dist_y = (ray.pos_y - ray_info->map_y) * ray_info->delta_dist_y;
	}
	else
	{
		ray_info->step_y = 1;
		ray_info->side_dist_y = (ray_info->map_y + 1.0 - ray.pos_y) * ray_info->delta_dist_y;
	}
}
